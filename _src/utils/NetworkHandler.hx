...
class NetworkHandler 
{
	...
	public function init(settings:Dynamic) 
	{
		...
		exhibitToken = settings.config.kontentum.exhibitToken;
		...
		restURLBase = restIP + "/" + restAPI +"/" + restID + "/" + exhibitToken;
	...
	public function startNet()
	{
		...
		{
			restURL = restURLBase + "/" +  StringTools.urlEncode(adapter.ip) + "/" + StringTools.urlEncode(adapter.mac) + "/" + StringTools.urlEncode(adapter.hostname) + "/" + StringTools.urlEncode(KontentumClient.buildDate);
